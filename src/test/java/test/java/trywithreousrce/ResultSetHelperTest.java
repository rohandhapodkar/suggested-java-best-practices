...
public class ResultSetHelperTest {
	...
	@Before
	public void initMocks() throws SQLException, IOException, ValidationException {
	...
	protected Class<? extends ResultSetHelper> getClassToSpy() {
		return ResultSetHelper.class;
	}
	...
	public void test_try_and_resources_throwing_exception() throws Exception {
		Mockito.doThrow(new ValidationException("ValidationException in copyResultSetToFile")).when(copyFile)
				.copyResultSetToStream(any(Connection.class), anyString(), any(Writer.class));
		Mockito.doThrow(new SQLException("Exception while closing connection")).when(con).close();
		Mockito.doThrow(new IOException("Exception while closing writer")).when(writer).close();

		Assertions.assertThatCode(() -> copyFile.copyResultSetToFile(query, fileName)).isInstanceOf(ValidationException.class)
				.hasMessage("ValidationException in copyResultSetToFile")
				.matches(throwable -> {
